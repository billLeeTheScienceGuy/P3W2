...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path copyPath, Edge extendBy) {
			...
			this.distance = copyPath.distance + extendBy.weight; // distance
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		...
		// add the original path to queue.
		queue.add(origin);
		while (!queue.isEmpty()) {
			// load the paths
			Path paths = queue.poll();

			// searching for the shortest path.
			if (paths.start.data.equals(start) && paths.end.data.equals(end)) {
				// if found, set it to shortestPath and break.
				shortestPath = paths;
				break;
			} else {
				Vertex currVert = paths.end;
				// visit the unvisited vertices
				if (!(visited.contains(currVert))) {
					for (int i = 0; i < currVert.edgesLeaving.size(); i++) {
						// add the edges leaving.
						queue.add(new Path(paths, currVert.edgesLeaving.get(i)));
					}
				}
				// add the current vertex as visited
				visited.add(currVert);

			}
		}
		// if shortest path does not exist, exception.
		if (shortestPath == null) {
			throw new NoSuchElementException("Shortest path does not exist");
